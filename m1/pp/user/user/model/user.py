...
def update(**user):
    ...
    if "new_password" in user:
        log.warn("changing password for '{}'".format(user['username']))
        new_password = user['new_password']
        user.pop('new_password')
        old = current.pop('password_hash')
        # Set the new password hash to store, replacing the current one:
        current['password_hash'] = pwtools.hash_password(new_password)
        print '2.1 current:\n', current
