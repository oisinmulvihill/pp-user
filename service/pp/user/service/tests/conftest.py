...
def user_svc(request):
    ...
    # recover the mongo and database we're to connect to:
    mongodb = request.getfuncargvalue('mongodb')
    cfg = dict(
    ...
    log.debug("test_server: mongo configuration '{}'".format(cfg))

    test_server = ServerRunner(cfg)
    ...
    test_server.api = UserService(uri=test_server.URI)
