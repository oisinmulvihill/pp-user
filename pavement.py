...
MODELDIR = path(
    os.path.abspath(os.path.join(CWD, "model"))
)

VALIDATEDIR = path(
    os.path.abspath(os.path.join(CWD, "validate"))
)

SERVICEDIR = path(
    os.path.abspath(os.path.join(CWD, "service"))
)

CLIENTDIR = path(
    os.path.abspath(os.path.join(CWD, "client"))
)
...
def runserver(options):
    ...
    cmd = ["pserve", "--reload", "development.ini", "--log-file", file_name]
...
def test(options):
    ...
    for dev_pkg in options.DEV_PKGS_IN_DEP_ORDER:
        # Change in profileservice dir (reset to here after last run):
        dev_pkg.chdir()
        easy.info("-- Changing to %s and testing --" % (dev_pkg))

        stdout = easy.sh("{python} setup.py test ".format(
                python=python,
            ),
            capture=True,
        )
